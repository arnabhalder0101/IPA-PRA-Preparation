...
public class IPA10{
       ...
       // sort second high salary
           for(int i = 0; i<e.length; i++){
               for(int j = i+1; j<e.length; j++){
                   if(e[i].getRating()< e[j].getRating()){
                       Employee k = e[i];
                       e[i] = e[j];
                       e[j] = k;
                   }
    ...
    public static void main(String args[]){
        ...
        int count = IPA10.findCountOfEmployeeUsingCompTransport(emp, branch);
        Employee res2 = IPA10.findEmployeeWithSecondHighestRating(emp);
